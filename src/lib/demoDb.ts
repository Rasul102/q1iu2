...
export async function getUserByUsername(username: string, network: NetworkMode) {
  const u = normalizeUsername(username);
  const { data, error } = await supabase
    .from('users')
    .select('id, username, wallet, network, created_at')
    .eq('network', network)
    .eq('username', u)
    .maybeSingle();
...
export async function getCreditsByUsername(username: string, network: NetworkMode) {
  const user = await getUserByUsername(username, network);
  if (!user) throw new Error('User not found. Ask them to connect wallet + create username first.');
  return getCredits(user.wallet, network);
}
